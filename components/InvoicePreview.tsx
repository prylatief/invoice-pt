            ...
            <div className="p-6 flex flex-col h-full flex-1">
                ...
                <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-right">
                        <h1 className="text-4xl font-extrabold tracking-tight mb-2 bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text text-transparent drop-shadow-sm">INVOICE</h1>
                    ...
                    <table className="w-full border-collapse rounded-xl overflow-hidden shadow-sm">
                        <thead>
                            <tr className="bg-gradient-to-r text-white shadow-md" style={{ backgroundImage: `linear-gradient(135deg, ${settings.brandColor}, ${settings.brandColor}dd)` }}>
                                <th className="py-2 px-4 text-left text-xs uppercase font-bold tracking-wider">Description</th>
                                <th className="py-2 px-4 text-center text-xs uppercase font-bold tracking-wider w-24">Qty</th>
                                <th className="py-2 px-4 text-right text-xs uppercase font-bold tracking-wider w-32">Price</th>
                                <th className="py-2 px-4 text-right text-xs uppercase font-bold tracking-wider w-32">Total</th>
                            </tr>
                        </thead>
                        <tbody className="text-gray-700">
                            {items.map((item, idx) => (
                                <tr key={item.id} className={`break-inside-avoid transition-colors hover:bg-gray-100/50 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                                    <td className="py-2 px-4 border-b border-gray-200/60 font-medium">{item.description}</td>
                                    <td className="py-2 px-4 text-center border-b border-gray-200/60">{item.quantity}</td>
                                    <td className="py-2 px-4 text-right border-b border-gray-200/60 text-gray-600">
                                        {formatCurrency(item.price, settings.currency)}
                                    </td>
                                    <td className="py-2 px-4 text-right border-b border-gray-200/60 font-bold text-gray-900">
                ...
                <div className="flex justify-end mb-4 break-inside-avoid">
                    <div className="w-2/3 md:w-1/2 lg:w-5/12 space-y-1 bg-gray-50/50 p-4 rounded-2xl border border-gray-200/60 shadow-sm">
                        ...
                        <div className="flex justify-between items-center py-2 bg-gradient-to-r from-cyan-50 to-blue-50 -mx-4 px-4 rounded-xl mt-1">
                            <span className="text-lg font-bold text-gray-800">Total</span>
                            <span className="text-xl font-extrabold text-cyan-700">{formatCurrency(total, settings.currency)}</span>
                        </div>
                ...
                <div className="mt-auto break-inside-avoid">
                    <div className="flex justify-between items-end gap-8 pb-3">
